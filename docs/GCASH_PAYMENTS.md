...
2. Update the files on the existing Raspberry Pi, including `ecorefill-pi/point_payments.py`, `ecorefill-pi/owner_refill_history.py`, and the complete `machine/` directory (especially `water_worker.py` and `owner_points.py`). Update the frontend alongside the Pi so both use owner-funded purchases. In the Pi's Python environment, install `firebase-admin` if it is missing. Start the updated machine service as usual:
...
- Owners use their existing `users/<ownerUid>.points` as their available balance, displayed live on Dashboard and Transactions. Missing balances default to zero. The Pi worker credits the actual `pointsUsed` only after confirmed successful dispensing, atomically with completion. Sessions store `ownerId` at reservation, `ownerPointsEarned`, and an `ownerPointsSettled` marker to prevent duplicate credits. Refill transactions record the owner's before/after balance. Failed/refunded refills earn no owner points.
- The balance card automatically calls the authenticated `syncOwnerRefillPoints` action to credit past completed refills, including legacy completed history. **Sync past refills** retries the same operation. Each request processes up to 25 candidate refills; the app follows the returned cursor to finish larger histories. Session and transaction records for one refill count once. Every existing source must agree on completed status and point cost; failed, unfinished, inconsistent, invalid, and already-settled records are skipped. Recorded ownership must match the signed-in owner. Records without an owner use the machine's current owner, checked again inside the transaction. No customer points, payment history, refill status, or original dates are changed.
- Historical credits atomically update the owner balance, mark source records settled, and create a deterministic `ownerRefillCredits/<hash>` receipt. The receipt prevents repeat credits even if an old transaction is later copied or reimported. This collection is server-only and is covered by the repository rules' default denial; never grant client writes. Retrying after a timeout or selling the credited points cannot restore those points through another history sync.
- Creating a purchase checks the owner's current balance. Points are not reserved while awaiting payment, so availability may change before review. Approval rechecks the balance, atomically deducts owner points, credits the buyer, updates the purchase, and creates `transactions/gcash_<purchaseId>` with both balances. Repeated/concurrent approvals cannot transfer twice or overspend the owner balance. Insufficient points leave the payment pending with no balance changes; owners must wait for more refill points or resolve the payment with the buyer. Rejection does not deduct points or issue a GCash refund.
